...
async def add_playlist_videos(video_key, voice_channel, author_id, channel, voice):
    ...
    for vi in video_ids:
        ...
        add_playlist(voice_channel, vi, title, length, author_id, audio_url, title, None)
            ...
            if bot_not_in_channel:
                ...
                if voice_channel == voice.channel:
                    ...
                    if url_search:
                        ...
                        title = information["title"]
                        length = information['duration']
                        add_playlist(voice_channel, video_key, title, length, message.author.id, audio_url, title, None)
...
@slash.slash(name="billboard", description="빌보드 차트를 출력합거나, 숫자를 입력하면 그 숫자에 해당하는 노래를 재생합니다", options=[create_option(name="number", description="빌보드 차트 숫자(중복 선택 가능 ex: 30, 20, 15)", option_type=3, required=False)])
...
async def on_reaction_add(reaction, member):
    ...
    if author_in_voice_chanel:
        if str(member.id) == str(keyword_search_dic[reaction.message.channel]["author"]):
            ...
            add_playlist(voice_channel, video_key, title, length, str(keyword_search_dic[reaction.message.channel]["author"]), audio_url, title, keyword_search_dic[reaction.message.channel]["search_keyword"])
            if not vc.is_playing():
                source = FFmpegPCMAudio(playlist[voice_channel]["list"][0]["audio_url"], **FFMPEG_OPTIONS)# converts the youtube audio source into a source discord can use
                voice.play(source)  # play the source
