...
###########################################
last_update = {'ED' : ['버전 GUI', '로또 GUI', '로또 기능 추가','로또 맨션', '로또 등록개수 수정','로또 중복순위 수정',  '펭구베타 기능'], 'HP' : [], 'F' : "False", 'ER': [], 'EVT' : ['로또']}
ver = "v1.86+EVT1+ED7"
"""다음 버전: 예외 줄이기//릴레이 소설 앨범만들기//GUI UPDATE"""
op_name = ['Windows 10', 'Codedis', 'MBP16', '폎폎/원래 본계 친삭&차단 부탁드립니다'] #최고 관리자
###########################################
event = True
...
async def on_message(message):
    global go, ver, last_update, op_name, lotto_list, event
    try:
    ...
    if message.content.startswith('펭구야 '):
        ...
        embed = discord.Embed(title=":scroll: 도움(명령어!)", description="펭구를 부를땐 '펭구야'라고 해주세요\n:가르치기 [물어보기];[대답]\n:잊어버려 [물어보기];[대답]\n:버전\n\n관리자 명령어\n:글로벌채팅 [켜기/끄기]\n:글로벌채팅 접속리스트\n:지우기 [개수]\n:배운것들\n:욕설리스트\n:공지 [만들기/보내기 [공지번호]]\n\n게임\n:소설 [보기/쓰기]\n:로또 [정보/등록 [번호]]", color=0x62c1cc)
        await message.channel.send(embed=embed)
                ...
                if len(num) == 6:
                    ...
                    if len(lotto_list[1][codes]) < 12 and not num in lotto_list[1][codes]:
                        ...
                        if num in lotto_list[1][codes]:
                            ...
                            embed = discord.Embed(title="로또 번호 등록", description="<@{0}>\n최대 3개까지 등록할 수 있어요(최대 등록 개수 초과)".format(message.author.id), color=0x62c1cc)
                            await message.channel.send(embed=embed)
                else:
